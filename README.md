...
- Docs content now includes the **v1.4.3** changelog, scoped runtime guidance, lifecycle manager docs, security practices, and canonical token guidance.
...
- Production docs load the pinned framework CDN bundle from `@v1.4.3`, while local preview resolves framework assets from `./dist` and refreshes them from the sibling framework repo when available.
